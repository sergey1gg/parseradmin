...
import { modifyTelegramLink } from "./utils/func";
    ...
    <main className="flex flex-col mt-2 text-sm">
      <div className=" grid grid-cols-2">
        <div className="">
          ...
          <button className=" bg-sky-500 p-1 text-white" onClick={() => { username !== "" && handleAddPublic(username, setSettingsList, setUsername, settingsList) }}>Добавить</button>
          <div className="">
...
const handleAddPublic = async (username: string, setSettingsList: React.Dispatch<any>, setUsername: React.Dispatch<string>, settingsList: any) => {
  ...
  if (duplicateUsernames.length > 0) {
    alert(`Следующие имена уже существуют: ${duplicateUsernames.join(', ')}`);
    return;
  }
  else {
    setUsername("")
